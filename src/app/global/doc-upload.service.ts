...
export class DocUploadService implements OnDestroy {
  ...
  uiProgress(processBar, stat = 0, create = false) {
    ...
    this.track.subscribe(ev => {
      console.log(ev, 'mmmmmm');//componentRef.instance.progress(ev);
    })
    // }
  }
  // vidtest;
  // track = new Subject<any>();
  // async videoUpTest(processBar) {
  //   let blob=new Blob()
  //   const options: FileUploadOptions = {
  //     fileKey: "attachment",
  //     fileName: 'test',
  ...
  //     headers: { Connection: "close" },
  //   };
  //   this.vidtest = this.transfer.create();
  //   //const fileTransfer: FileTransferObject = this.transfer.create();
  //   const progressComponent: ComponentFactory<ProgressWidgetComponent> = this.compFactoryResolver.resolveComponentFactory(ProgressWidgetComponent);
  //   const hostViewContainer: ViewContainerRef = processBar.viewContainerRef;
  //   hostViewContainer.clear();
  ...
  //   componentRef.instance.imgTotalCount = 1;
  //   // const success = await 
  //   this.vidtest.upload(this.file.externalApplicationStorageDirectory + 'test.mp4', this.postUrl, options).then(val => { console.log(val); })
  //   this.vidtest.onProgress((data) => {
  //     console.log(`current ${data.loaded} / total ${data.total}`);
  //     let per = Math.floor((data.loaded * 100 / data.total));
  //     componentRef.instance.progress(per)
  //   })
  //   // console.log(success, 'test success');
  //   try {
  //   } catch (error) {
  //     console.log(error, 'file tranfer');
  //   }
  // }
